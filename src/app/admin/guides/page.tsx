      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 md:gap-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-black text-slate-900 md:text-4xl">매듭 가이드 관리</h1>
          </div>
          <div className="flex flex-wrap gap-3 sm:self-start">
            ...
            <Link href="/guides" className="inline-flex min-h-14 items-center justify-center rounded-2xl border border-slate-300 bg-white px-5 py-3 text-base font-bold text-slate-700 transition hover:border-slate-500">
              작업자 가이드 보기
            </Link>
          </div>
        </div>

        <AdminGuidesClient initialGuides={guides} dbConfigured={isGuidesDbConfigured()} />
      </div>
    </main>
  )
}
